...
class _AboutPageState extends State<AboutPage> {
  ...
  Future<void> _showTabletCardScaleDialog() async {
    double scale = Pref.tabletCardScale;
    final result = await showDialog<double>(
      context: context,
      builder: (context) => StatefulBuilder(
        builder: (context, setState) => AlertDialog(
          title: const Text('平板视频卡片放大倍数'),
          content: Column(
            mainAxisSize: MainAxisSize.min,
            children: [
              Text('${scale.toStringAsFixed(2)} 倍'),
              Slider(
                value: scale,
                min: 1.0,
                max: 1.5,
                divisions: 10,
                label: '${scale.toStringAsFixed(2)} 倍',
                onChanged: (value) => setState(() => scale = value),
              ),
              const Text('仅影响平板横向视频卡片，重启应用后生效'),
            ],
          ),
          actions: [
            TextButton(
    ...
    if (result != null && result != Pref.tabletCardScale) {
      await GStorage.setting.put(SettingBoxKey.tabletCardScale, result);
...
Commit Hash: ${BuildConfig.commitHash}''',
          ...
          ListTile(
            onTap: _showTabletCardScaleDialog,
            leading: const Icon(Icons.photo_size_select_large_outlined),
            title: const Text('平板视频卡片放大倍数'),
            subtitle: Text(
              '${Pref.tabletCardScale.toStringAsFixed(2)} 倍，重启应用后生效',
              style: subTitleStyle,
            ),
